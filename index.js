...
function calculateAllInventoryPrice() {
  return items.reduce((total, item)=> item.inventory*item.price+total,0)
...
function getItemPriceByName(name) {
  thing = items.find((item => item.name===name))
  thing = thing.price
  return thing
...
function logCartItems() {
    ...
    idArray.push(parseInt(cart[i]))};
    console.log(idArray)
    for(let i=0; i<idArray.length;i++){
    console.log(items[idArray[i]])
  }
...
idArray.push(parseInt(cart[i]))};
for(let i=0; i<idArray.length;i++){
  total.push(items[idArray[i]])
  }
  // for(let i=0; i<total.length;i++){
  //   console.log(total.price[i])
  //   }
  for(let i=0; i<total.length;i++){
  //  console.log(total[i].price)
  //  console.log(total[i].inventory)
 final += total[i].price * total[i].inventory
